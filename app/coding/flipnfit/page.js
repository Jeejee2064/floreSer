...
// ── LEVELS ─────────────────────────────────────────────────────────
const LEVELS = [
  { hint:'The tunnel is 3 units tall. Rotate the rect first, then slide through!', start:{x:1,y:9,rotation:0}, target:{x:15,y:9,rotation:0}, obstacles:[{x:7,y:0,w:1,h:8.5},{x:7,y:11.5,w:1,h:8.5}] },
  { hint:'The tunnel is 3 units tall. Rotate the rect first, then slide through!', start:{x:1,y:9,rotation:90}, target:{x:12,y:9,rotation:0}, obstacles:[{x:7,y:0,w:1,h:8.5},{x:7,y:11.5,w:1,h:8.5}] },
  { hint:'A wall blocks the way down. Rotate 90° then drop through the gap.', start:{x:8,y:1,rotation:0}, target:{x:8,y:13,rotation:90}, obstacles:[{x:0,y:8,w:8.5,h:1},{x:11.5,y:8,w:8.5,h:1}] },
  { hint:'A wall blocks the way down. Rotate 90° then drop through the gap.', start:{x:8,y:1,rotation:0}, target:{x:10,y:15,rotation:90}, obstacles:[{x:0,y:8,w:10.5,h:1},{x:13.5,y:8,w:8.5,h:1}] },
  { hint:'A wall blocks the way down. Rotate 90° then drop through the gap.', start:{x:8,y:1,rotation:0}, target:{x:10,y:15,rotation:0}, obstacles:[{x:0,y:8,w:10.5,h:1},{x:13.5,y:8,w:8.5,h:1}] },
  { hint:'A wall blocks the way down. Rotate 90° then drop through the gap.', start:{x:8,y:1,rotation:0}, target:{x:1,y:10,rotation:0}, obstacles:[{x:0,y:8,w:10.5,h:1},{x:13.5,y:8,w:8.5,h:1}] },
  { hint:'Two tunnels. Slide right flat, rotate, drop, rotate back, slide right.', start:{x:1,y:4,rotation:0}, target:{x:14,y:14,rotation:0}, obstacles:[{x:7,y:0,w:1,h:6.5},{x:7,y:9.5,w:1,h:4},{x:12,y:7,w:1,h:13}] },
  { hint:'Navigate around the corner: go right, then rotate to fit, then go down.', start:{x:1,y:1,rotation:0}, target:{x:14,y:13,rotation:90}, obstacles:[{x:3,y:7,w:13,h:1},{x:13,y:0,w:1,h:8},{x:9,y:12,w:1,h:8}] },
];
...
function calcPoints(t) {
  if (t<=1) return 10; if (t===2) return 7; if (t===3) return 5; if (t===4) return 3; return 1;
}
...
function LeaderboardModal({ onClose, highlightName }) {
  ...
  useEffect(() => {
    supabase.from('flip_and_fit_leaderboard').select('*')
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 shrink-0">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">Top 20 campaign scores</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
          ...
          {rows.map((row, i) => {
            ...
            return (
              <div key={i}
                className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all ${isMe ? 'bg-yellow-500/15 border border-yellow-500/40' : 'bg-slate-800/50 border border-slate-700/40'}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500">{row.levels_completed} levels</p>
                </div>
...
function ScoreSubmitModal({ score, levelsCompleted, onDone }) {
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from('flip_and_fit_scores').insert({
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <h2 className="text-2xl font-black text-emerald-400 italic uppercase mb-1">Cleared!</h2>
        <p className="text-slate-400 text-sm mb-1">All {levelsCompleted} levels done</p>
...
// ── COMMUNITY MODAL ────────────────────────────────────────────────
function CommunityModal({ onClose, onPlay }) {
  const [levels, setLevels] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  useEffect(() => {
    supabase.from('flip_and_fit_levels').select('*').order('created_at', { ascending: false })
      .then(({ data, error }) => { setLoading(false); if (error) setError(error.message); else setLevels(data||[]); });
  }, []);
  ...
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <motion.div
        ...
        exit={{ opacity:0, scale:0.92, y:16 }}
        transition={{ type:'spring', stiffness:280, damping:22 }}
        className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[80vh]"
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 shrink-0">
          <div>
            <h2 className="text-base font-black text-orange-500 italic uppercase">Community Levels</h2>
            <p className="text-[10px] text-slate-500 mt-0.5">Levels made by players like you</p>
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-white text-lg font-bold">✕</button>
        </div>

        <div className="flex-1 overflow-y-auto p-4">
          {loading && <div className="text-center py-10 text-slate-500 text-sm">Loading levels…</div>}
          ...
          {!loading && !error && levels.length === 0 && (
            <div className="text-center py-10 text-slate-600 text-sm italic">
              ...
              <a href="/coding/flipnfit/level-maker" className="text-orange-500 hover:text-orange-400 font-bold not-italic">Be the first to make one →</a>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            {levels.map(lvl => (
              <div key={lvl.id}
                className="bg-slate-800/60 hover:bg-slate-800 border border-slate-700/50 hover:border-orange-500/40 rounded-xl overflow-hidden transition-all group flex flex-col">
                {/* mini preview */}
                <div className="bg-[#01040a] border-b border-slate-700/50 p-1">
                  <svg viewBox="0 0 20 20" className="w-full aspect-square">
                    {(lvl.obstacles||[]).map((o,i) => <rect key={i} x={o.x} y={o.y} width={o.w} height={o.h} fill="#334155" />)}
                    <rect x={lvl.target_x} y={lvl.target_y} width={4} height={2}
                      fill="none" stroke="#4ADE80" strokeWidth="0.5" strokeDasharray="0.8,0.6" opacity={0.7}
                      transform={`rotate(${lvl.target_rotation} ${lvl.target_x+2} ${lvl.target_y+1})`} />
                    <rect x={lvl.start_x} y={lvl.start_y} width={4} height={2} fill="#F97316"
                      transform={`rotate(${lvl.start_rotation} ${lvl.start_x+2} ${lvl.start_y+1})`} />
                  </svg>
                </div>
                {/* info + play */}
                <div className="p-3 flex flex-col gap-2 flex-1">
                  <div className="flex-1">
                    <p className="font-black text-white text-xs truncate leading-tight">{lvl.level_name}</p>
                    <p className="text-[10px] text-slate-500 truncate">by {lvl.creator_name}</p>
                    {lvl.hint && <p className="text-[9px] text-slate-600 italic truncate mt-1">"{lvl.hint}"</p>}
                  </div>
                  <button onClick={() => onPlay(lvl)}
                    className="w-full bg-orange-600 hover:bg-orange-500 text-white font-black text-[10px] uppercase px-2 py-1.5 rounded-lg transition-all">
                    Play →
                  </button>
                </div>
              </div>
        ...
        <div className="px-5 py-3 border-t border-slate-800 shrink-0">
          <a href="/coding/flipnfit/level-maker"
            className="block text-center text-[11px] text-orange-500 hover:text-orange-400 font-bold uppercase tracking-widest transition-colors">
...
function dbRowToLevel(row) {
  return {
    hint: row.hint||'',
    start:  {x:+row.start_x,  y:+row.start_y,  rotation:+row.start_rotation},
    target: {x:+row.target_x, y:+row.target_y, rotation:+row.target_rotation},
    obstacles: (row.obstacles||[]).map(o=>({x:+o.x,y:+o.y,w:+o.w,h:+o.h})),
    _name: row.level_name, _creator: row.creator_name,
  };
}
...
export default function ObstacleCourse() {
  ...
  // 'idle' | 'submitting' — shown after campaign ends
  const [scorePhase,    setScorePhase]    = useState('idle');
  const [finalScore,    setFinalScore]    = useState(0);

  const commandListRef = useRef(null);
  const lvl = mode === 'custom' ? customLevel : LEVELS[levelIdx];
  ...
  function startGame() {
    setMode('builtin'); setLevelIdx(0); setCommands([]); resetDisplay();
    setAttempted(false); setSuccess(false); setCollision(false);
    setTries(0); setTotalPts(0); setModal(null);
    setShowCommunity(false); setShowLeaderboard(false);
    setScorePhase('idle');
    setScreen('game');
  }

  function playCustomLevel(row) {
    setCustomLevel(dbRowToLevel(row)); setMode('custom');
    setCommands([]); resetDisplay();
    setAttempted(false); setSuccess(false); setCollision(false);
    setTries(0); setModal(null); setShowCommunity(false);
    setScreen('game');
  }

  function addCommand(type)  { if (isRunning) return; setAttempted(false); setCollision(false); setCommands(p=>[...p,{id:crypto.randomUUID(),type,value:0}]); }
  ...
  function removeCommand(id) { if (isRunning) return; setCommands(p=>p.filter(c=>c.id!==id)); }
  function reorderCommands(from,to) {
    if (isRunning) return; setAttempted(false); setCollision(false);
    const r=[...commands]; const [m]=r.splice(from,1); r.splice(to,0,m); setCommands(r);
  }
  function resetLevel() {
    if (isRunning) return; setCommands([]); resetDisplay();
    setAttempted(false); setSuccess(false); setCollision(false); setTries(0);
  }

  function nextLevel(currentPts) {
    if (mode === 'custom') { setScreen('menu'); return; }
    const next = levelIdx + 1;
    if (next >= LEVELS.length) {
      setFinalScore(currentPts);
  ...
  async function handleRun() {
    if (isRunning || commands.length===0 || !lvl) return;
    setIsRunning(true); setAttempted(false); setSuccess(false); setCollision(false);
    ...
    for (const cmd of commands) {
      ...
      if (cmd.type==='x') {
        const substeps=Math.max(8,Math.ceil(Math.abs(total)*4)), inc=total/substeps;
        let safeX=cX, hit=false;
        for (let i=1;i<=substeps;i++) { const tX=cX+inc*i; if(checkCollision(start.x+tX,start.y+cY,start.rotation+cRot,obstacles)){hit=true;break;} safeX=tX; }
        await springTo(cX,safeX,setDispX); cX=safeX;
        ...
        const substeps=Math.max(8,Math.ceil(Math.abs(total)*4)), inc=total/substeps;
        let safeY=cY, hit=false;
        for (let i=1;i<=substeps;i++) { const tY=cY+inc*i; if(checkCollision(start.x+cX,start.y+tY,start.rotation+cRot,obstacles)){hit=true;break;} safeY=tY; }
        await springTo(cY,safeY,setDispY); cY=safeY;
        if (hit) { setCollision(true);setAttempted(true); await delay(500); await Promise.all([springTo(cX,0,setDispX),springTo(cY,0,setDispY),springTo(cRot,0,setDispRot)]); setIsRunning(false); return; }
      } else if (cmd.type==='rotation') {
        const substeps=Math.max(8,Math.ceil(Math.abs(total)/5)), inc=total/substeps;
        let safeRot=cRot, hit=false;
        for (let i=1;i<=substeps;i++) { const tR=cRot+inc*i; if(checkCollision(start.x+cX,start.y+cY,start.rotation+tR,obstacles)){hit=true;break;} safeRot=tR; }
        await springTo(cRot,safeRot,setDispRot); cRot=safeRot;
        if (hit) { setCollision(true);setAttempted(true); await delay(500); await Promise.all([springTo(cX,0,setDispX),springTo(cY,0,setDispY),springTo(cRot,0,setDispRot)]); setIsRunning(false); return; }
      }
      await delay(380);
    }
    ...
    // win check
    const pCorners = getRectCorners(start.x+cX,start.y+cY,4,2,start.rotation+cRot);
    const tCorners = getRectCorners(target.x,target.y,4,2,target.rotation);
    let matchCount=0; const used=new Set();
    for (let i=0;i<4;i++) { const pc=pCorners[i]; for(let j=0;j<4;j++){ if(used.has(j))continue; const tc=tCorners[j]; if(Math.abs(pc.x-tc.x)<0.35&&Math.abs(pc.y-tc.y)<0.35){matchCount++;used.add(j);break;} } }
    ...
    if (isWin) {
      ...
      await delay(1800); setModal(null);
      nextLevel(newTotal);
    } else {
      await delay(500);
  ...
  // ── DERIVED ──────────────────────────────────────────────────────
  const {start,target,obstacles} = lvl||LEVELS[0];
  const playerX=start.x+dispX, playerY=start.y+dispY, playerRot=start.rotation+dispRot;
  const playerCx=playerX+2, playerCy=playerY+1;
  ...
  if (screen==='menu') return (
    <div className="fixed inset-0 bg-slate-950 text-white flex items-center justify-center font-mono">
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="text-center max-w-md w-full px-6">
        ...
        <div className="bg-slate-900/50 rounded-2xl p-4 mb-4 border border-slate-800">
          ...
          <div className="space-y-3">
            <motion.div className="flex items-center gap-3 bg-blue-900/20 border border-blue-500/30 rounded-xl p-3" animate={{x:[0,8,-8,0]}} transition={{repeat:Infinity,duration:3,ease:'easeInOut'}}>
              <div className="w-8 h-8 bg-blue-500/20 rounded-lg flex items-center justify-center text-blue-400 font-bold">X</div>
              <div className="flex-1 text-left"><span className="text-blue-400 font-black text-sm">translateX()</span><span className="text-slate-500 text-xs ml-2">← → left or right</span></div>
              <motion.div className="w-6 h-3 bg-blue-500 rounded-sm" animate={{x:[0,8,-8,0]}} transition={{repeat:Infinity,duration:3,ease:'easeInOut'}} />
            </motion.div>
            <motion.div className="flex items-center gap-3 bg-purple-900/20 border border-purple-500/30 rounded-xl p-3" animate={{y:[0,-5,5,0]}} transition={{repeat:Infinity,duration:3,ease:'easeInOut',delay:0.4}}>
              <div className="w-8 h-8 bg-purple-500/20 rounded-lg flex items-center justify-center text-purple-400 font-bold">Y</div>
              <div className="flex-1 text-left"><span className="text-purple-400 font-black text-sm">translateY()</span><span className="text-slate-500 text-xs ml-2">↑ ↓ up or down</span></div>
              <motion.div className="w-6 h-3 bg-purple-500 rounded-sm" animate={{y:[0,-5,5,0]}} transition={{repeat:Infinity,duration:3,ease:'easeInOut',delay:0.4}} />
            </motion.div>
            <motion.div className="flex items-center gap-3 bg-emerald-900/20 border border-emerald-500/30 rounded-xl p-3" animate={{rotate:[0,30,-30,0]}} transition={{repeat:Infinity,duration:4,ease:'easeInOut',delay:0.8}}>
              <div className="w-8 h-8 bg-emerald-500/20 rounded-lg flex items-center justify-center text-emerald-400 font-bold">↻</div>
              <div className="flex-1 text-left"><span className="text-emerald-400 font-black text-sm">rotate()</span><span className="text-slate-500 text-xs ml-2">↻ ↺ clockwise or counter</span></div>
              <motion.div className="w-6 h-3 bg-emerald-500 rounded-sm origin-center" animate={{rotate:[0,45,-45,0]}} transition={{repeat:Infinity,duration:4,ease:'easeInOut',delay:0.8}} />
            </motion.div>
          </div>
        ...
        <p className="text-slate-200 text-xs mb-4"><span className="text-yellow-400">⭐ Points per level:</span> 1st try=10 • 2nd=7 • 3rd=5 • 4th=3 • 5th+=1</p>
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <button onClick={()=>setShowLeaderboard(true)}
            className="py-3 bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/30 rounded-xl font-black text-xs uppercase text-yellow-400 transition-all">
            🏆 Scores
          </button>
      ...
      <AnimatePresence>
        {showCommunity   && <CommunityModal onClose={()=>setShowCommunity(false)} onPlay={playCustomLevel} />}
    ...
    <div className="fixed inset-0 bg-slate-950 text-white flex flex-col md:flex-row font-mono overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-slate-950 relative select-none">
        <div className="relative w-full aspect-square max-w-[550px] border-2 border-slate-800 bg-[#01040a]">
          ...
          <svg viewBox={`0 0 ${GRID} ${GRID}`} className="w-full h-full overflow-visible">
            ...
            {obstacles.map((o,i)=><rect key={i} x={o.x} y={o.y} width={o.w} height={o.h} fill="#334155" stroke="#475569" strokeWidth="0.1"/>)}
            <rect x={target.x} y={target.y} width={4} height={2} fill="none" stroke="#4ADE80" strokeWidth="0.25" strokeDasharray="0.5,0.4" opacity={0.6} transform={targetXform}/>
            <rect x={playerX} y={playerY} width={4} height={2}
              fill={collision?'#EF4444':success?'#4ADE80':'#F97316'}
              stroke={collision?'#FCA5A5':success?'#fff':'none'} strokeWidth={0.2} transform={playerXform}
              style={{filter:collision?'drop-shadow(0 0 4px rgba(239,68,68,0.8))':success?'drop-shadow(0 0 6px rgba(74,222,128,0.7))':'none'}}
            />
            {success  && <motion.rect x="0" y="0" width={GRID} height={GRID} fill="#4ADE80" initial={{opacity:0.15}} animate={{opacity:0}} transition={{duration:1.2}}/>}
            ...
            {modal && (
              <motion.div initial={{opacity:0,scale:0.7,y:10}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:0.8,y:-10}} transition={{type:'spring',stiffness:300,damping:18}} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="bg-slate-900/95 border-2 border-emerald-500/60 rounded-2xl px-10 py-8 text-center shadow-2xl">
                  ...
                  <div className="text-slate-500 text-xs uppercase tracking-widest">{modal.tries===1?'First try!':modal.tries===2?'2nd attempt':modal.tries===3?'3rd attempt':`${modal.tries} attempts`}</div>
                  {isCustom && <div className="text-slate-600 text-[10px] mt-2">Returning to menu…</div>}
                </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <div className="w-full md:w-[400px] bg-slate-900 border-l border-white/10 p-5 flex flex-col overflow-hidden shadow-2xl">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={isCustom?'custom':levelIdx} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-12}} className="flex-1 flex flex-col min-h-0">
            <div className="bg-black/40 p-3 rounded-xl border border-orange-500/20 mb-3 flex-shrink-0">
              <span className="text-[10px] text-slate-500 uppercase font-bold tracking-widest block">{isCustom?'🌐 Community Level':`Level ${levelIdx+1}`}</span>
              {isCustom && <span className="text-xs text-orange-400 font-bold">{lvl._name} <span className="text-slate-500 font-normal">by {lvl._creator}</span></span>}
              {lvl.hint && <p className="text-[10px] text-slate-500 italic mt-1">{lvl.hint}</p>}
            </div>
                ...
                <AnimatePresence initial={false}>
                  {commands.map((cmd,idx)=>{const meta=CMD_META[cmd.type];const col=C[cmd.type];return(
                    <motion.div key={cmd.id}
                      ...
                      onDragEnd={(_,info)=>{const ITEM_HEIGHT=48;const moveBy=Math.round(info.offset.y/ITEM_HEIGHT);const newIndex=Math.max(0,Math.min(commands.length-1,idx+moveBy));if(newIndex!==idx)reorderCommands(idx,newIndex);}}
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button onClick={()=>setVal(cmd.id,cmd.value-meta.step)} disabled={isRunning} className={`bg-slate-700 ${col.hov} text-white font-black w-7 h-7 rounded flex items-center justify-center transition-all disabled:opacity-30 text-sm`}>−</button>
                        <input type="number" value={cmd.value} onChange={e=>setVal(cmd.id,e.target.value)} disabled={isRunning} className={`w-12 bg-slate-900 font-black text-sm outline-none ${col.text} text-center rounded py-1 border ${col.border} disabled:opacity-40`}/>
                        <button onClick={()=>setVal(cmd.id,cmd.value+meta.step)} disabled={isRunning} className={`bg-slate-700 ${col.hov} text-white font-black w-7 h-7 rounded flex items-center justify-center transition-all disabled:opacity-30 text-sm`}>+</button>
                      </div>
            ...
            <AnimatePresence>
              {collision && !isRunning && (
                <motion.div initial={{opacity:0,y:4}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="mb-3 bg-red-500/10 border border-red-500/40 rounded-xl p-3 text-center flex-shrink-0">
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              <button onClick={resetLevel} disabled={isRunning} className="px-4 py-3.5 bg-slate-800 hover:bg-slate-700 disabled:opacity-30 rounded-xl font-bold text-slate-300 transition-all text-lg">↺</button>
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
