...
const SHAPE_OPTIONS = [
  ...
  { id: 'rect',    emoji: '🟦', label: 'Rect'     },
  { id: 'line',    emoji: '📏', label: 'Line'     },
  { id: 'polygon', emoji: '🔺', label: 'Triangle' },
];

export default function DrawWithCode() {
  // screens: 'menu' | 'setup-choose' | 'setup-write' | 'game' | 'victory'
  ...
  useEffect(() => {
    if (penaltyTime > 0) {
      const timer = setTimeout(() => setPenaltyTime(p => p - 1), 1000);
  ...
  const handleCheck = () => {
    ...
    let allGood = true;
    const check = (key, val) => {
      const ok = Number(answer[key]) === val;
      results[key] = ok ? 'correct' : 'wrong';
      if (!ok) allGood = false;
    };
    if (target.type === 'polygon') {
      target.points.forEach((p, i) => { check(`p${i}x`, p.x); check(`p${i}y`, p.y); });
    } else if (target.type === 'line') {
      check('x1', target.x1); check('y1', target.y1); check('x2', target.x2); check('y2', target.y2);
    } else {
      Object.keys(target).filter(k => typeof target[k] === 'number').forEach(k => check(k, target[k]));
    }
    ...
    if (allGood) {
      ...
      const next = score + 1;
      setScore(next);
      setTimeout(() => { setFeedback(null); if (next >= 25) setScreen('victory'); else newRound(); }, 1000);
    } else {
      setIsShaking(true); setFeedback('wrong');
      setTimeout(() => { setIsShaking(false); if (mode === 'write') setTimeout(() => setFeedback(null), 1500); else setPenaltyTime(5); }, 400);
    }
  };

  const handleGridClick = (s) => {
    if (penaltyTime > 0 || feedback) return;
    if (mode === 'choose') {
      if (s.id === target?.id) {
        setFeedback('correct'); setScore(n => n + 1);
  ...
  const InputBox = ({ k, label }) => (
    <input placeholder={label || k} type="number" value={answer[k] || ''}
      onChange={e => { setAnswer({...answer, [k]: e.target.value}); const r = {...checkedResults}; delete r[k]; setCheckedResults(r); }}
      className={`w-12 h-12 bg-black border-2 text-center font-bold rounded-lg text-lg outline-none transition-all
        ${checkedResults[k] === 'correct' ? 'border-green-500 shadow-[0_0_10px_#22c55e]' :
          checkedResults[k] === 'wrong' ? 'border-red-500 animate-pulse shadow-[0_0_10px_#ef4444]' :
          'border-slate-700 focus:border-pink-500'}`}
    />
  );

  // ── SETUP SCREEN (shape selector + mode) ─────────────────────────
  const SetupScreen = ({ chosenMode }) => (
    <motion.div key="setup" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
      className="m-auto w-full max-w-sm px-6 py-8"
    >
      <button onClick={() => setScreen('menu')} className="flex items-center gap-1 text-slate-600 hover:text-white text-xs font-bold uppercase mb-6 transition-colors">
        ← Back
      </button>

      <div className="mb-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-pink-500/30 bg-pink-500/10 mb-3">
          <span className="text-pink-400 font-black text-xs uppercase tracking-widest">
            {chosenMode === 'choose' ? '👁 Click Mode' : '✏️ Write Mode'}
          </span>
        </div>
        <h2 className="text-3xl font-black text-white mb-1">Pick your shapes</h2>
        <p className="text-slate-500 text-sm">Choose which shape types will appear in the game.</p>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-8">
        {SHAPE_OPTIONS.map(({ id, emoji, label }) => {
          const on = enabledShapes.includes(id);
          return (
            <button key={id}
              onClick={() => setEnabledShapes(prev =>
                prev.includes(id) ? (prev.length > 1 ? prev.filter(x => x !== id) : prev) : [...prev, id]
              )}
              className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all font-bold text-sm ${
                on ? 'border-pink-500 bg-pink-500/15 text-white shadow-lg shadow-pink-900/20'
                   : 'border-slate-800 text-slate-600 hover:border-slate-600'
              }`}
            >
              <span className="text-2xl">{emoji}</span>
              <span>{label}</span>
              {on && <span className="ml-auto text-pink-400 text-xs">✓</span>}
            </button>
          );
        })}
      </div>
      ...
      <motion.button
        whileTap={{ scale: 0.97 }}
        onClick={() => { setMode(chosenMode); setScore(0); setScreen('game'); }}
        className="w-full py-5 bg-pink-600 hover:bg-pink-500 rounded-2xl font-black text-xl transition-all shadow-lg shadow-pink-900/30"
      >
        LET'S GO →
      </motion.button>
    </motion.div>
      ...
      <AnimatePresence mode="wait">
        ...
        {screen === 'menu' && (
          <motion.div key="menu" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
            className="m-auto w-full max-w-sm px-6 text-center"
          >
            <Link href="/coding" className="inline-flex items-center gap-1 text-slate-700 hover:text-slate-400 text-xs font-bold uppercase mb-8 transition-colors">
              ← Arcade
            </Link>

            <div className="text-6xl mb-4">🎯</div>
            <h1 className="text-6xl font-black mb-2 text-pink-500 italic uppercase">Code Draw</h1>
            <p className="text-slate-400 text-sm mb-10 leading-relaxed">
              Read SVG code and find the shape — or write the coordinates yourself!
            </p>

            <div className="space-y-3">
              <motion.button whileTap={{ scale: 0.97 }}
                onClick={() => setScreen('setup-choose')}
                className="w-full py-5 bg-pink-600 hover:bg-pink-500 rounded-2xl font-black text-xl transition-all shadow-lg shadow-pink-900/30 flex flex-col items-center gap-1"
              >
                <span>👁 Click Mode</span>
                <span className="font-normal text-pink-200/70 text-sm">Tap the shape that matches the code</span>
              </motion.button>
              <motion.button whileTap={{ scale: 0.97 }}
                onClick={() => setScreen('setup-write')}
                className="w-full py-5 border-2 border-white/20 hover:bg-white/5 rounded-2xl font-black text-xl transition-all flex flex-col items-center gap-1"
              >
                <span>✏️ Write Mode</span>
                <span className="font-normal text-slate-500 text-sm">Type the coordinates yourself</span>
              </motion.button>
            </div>
          </motion.div>
        )}
        ...
        {/* ── SETUP SCREENS ── */}
        {screen === 'setup-choose' && <SetupScreen chosenMode="choose" />}
        {screen === 'setup-write'  && <SetupScreen chosenMode="write"  />}
        ...
        {screen === 'game' && target && (
          <motion.div key="g" className="w-full h-full flex flex-col">
            <div className="flex justify-between items-center px-4 py-3 bg-slate-900 border-b border-white/10 shrink-0">
              <button onClick={() => setScreen('menu')} className="bg-slate-800 hover:bg-slate-700 px-4 py-2 rounded-lg font-bold text-xs transition-colors">EXIT</button>
            ...
            <motion.div animate={isShaking ? { x: [-5, 5, -5, 5, 0] } : {}}
              className="px-4 py-2 bg-slate-900/60 flex items-center justify-center gap-4 shrink-0 overflow-x-auto"
            >
              {mode === 'write' ? (
                ...
                <p className="text-slate-500 italic text-sm font-bold py-2">Click the shape that matches the code above!</p>
              )}
            </motion.div>
            ...
            <div className="flex-1 flex items-center justify-center p-6 relative bg-slate-950 overflow-hidden">
              <div className="relative h-full aspect-square border-2 border-slate-800 bg-[#01040a]">
                {Array.from({ length: GRID + 1 }).map((_, i) => (
                  <div key={i} className="absolute text-[10px] font-bold text-slate-600"
                    style={{ left: `${(i / GRID) * 100}%`, top: '-22px', transform: 'translateX(-50%)' }}>{i}</div>
                ))}
                {Array.from({ length: GRID + 1 }).map((_, i) => (
                  <div key={i} className="absolute text-[10px] font-bold text-slate-600"
                    style={{ left: '-22px', top: `${(i / GRID) * 100}%`, transform: 'translateY(-50%)' }}>{i}</div>
                ))}
                ...
                <AnimatePresence>
                  {feedback === 'correct' && (
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 1.5, opacity: 0 }}
                  ...
                  {feedback === 'wrong' && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      className="absolute inset-0 flex flex-col items-center justify-center bg-red-950/80 z-50 p-6">
                      {mode === 'choose' ? (
                        <>
                          <h2 className="text-4xl font-black text-red-500 mb-2 italic">LOCKED!</h2>
                          <div className="relative flex items-center justify-center">
                            <svg className="w-24 h-24 transform -rotate-90">
                              <circle cx="48" cy="48" r="42" stroke="white" strokeOpacity="0.1" strokeWidth="6" fill="transparent" />
                              <motion.circle cx="48" cy="48" r="42" stroke="#ef4444" strokeWidth="6" fill="transparent"
                                strokeDasharray="264" initial={{ strokeDashoffset: 0 }} animate={{ strokeDashoffset: 264 }}
        ...
        {screen === 'victory' && (
          <motion.div key="v" initial={{ scale: 0.5 }} animate={{ scale: 1 }} className="m-auto text-center p-12 bg-slate-900 border-4 border-yellow-400 rounded-3xl">
            ...
            <h2 className="text-6xl font-black text-yellow-400 italic mb-8">LEGENDARY!</h2>
            <div className="flex gap-4 justify-center flex-wrap">
              <button onClick={() => setScreen('menu')} className="px-10 py-5 bg-white text-black font-black rounded-xl text-xl active:scale-95 transition-all">PLAY AGAIN</button>
              <Link href="/coding" className="px-10 py-5 border-2 border-white/30 font-black rounded-xl text-xl hover:bg-white/10 transition-all">← ARCADE</Link>
            </div>
          </motion.div>
        )}
